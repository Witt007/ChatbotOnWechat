...
class PuppetWechat4u extends PUPPET.Puppet {
    ...
    monkeyPatch(wechat4u) {
        ...
        this.monkeyPatchHook(wechat4u, 'syncCheck', () => {
            wechaty_puppet_1.log.silly('PuppetWechat4u', 'monkeyPatch() monkeyPatchHook() wechat4u.syncCheck()');
        ...
        wechat4u['checkPolling'] = () => {
            wechaty_puppet_1.log.silly('PuppetWechat4u', 'monkeyPatch() wechat4u.checkPolling()');
            if (this.state.inactive()) {
                return;
            }
            wechat4u.notifyMobile()
                .catch((err) => {
                wechaty_puppet_1.log.warn('PuppetWechat4u', 'monkeyPatch() wechat4u.checkPolling() notifyMobile() exception: %s', err);
    ...
    monkeyPatchHook(wechat4u, func, hookFunc) {
        wechaty_puppet_1.log.verbose('PuppetWechat4u', 'monkeyPatchHook(wechat4u, %s, func)', func);
        const funcOrig = wechat4u[func];
        function funcNew() {
            wechaty_puppet_1.log.silly('PuppetWechat4u', 'monkeyPatchHook(wechat4u, %s, func) funcNew()', func);
            hookFunc();
            return funcOrig.call(this);
        }
        wechat4u[func] = funcNew;
    ...
    initHookEvents(wechat4u) {
        ...
        wechat4u.on('message', async (msg) => {
            ...
            if (msg.CreateTime < this.startTime) {
                wechaty_puppet_1.log.warn('PuppetWechat4u', 'initHookEvents() wechat4u.on(message) is history message: %s', JSON.stringify(msg));
                return;
            }
    ...
    async messageFile(id) {
        ...
        switch (payload.type) {
            ...
            case PUPPET.types.Message.Image:
                ...
                return file_box_1.FileBox.fromStream((await this.wechat4u.getMsgImg(rawPayload.MsgId)).data, filename);
            case PUPPET.types.Message.Audio: {
                /**
                 * 语音消息
                 */
                const audioFileBox = file_box_1.FileBox.fromStream((await this.wechat4u.getVoice(rawPayload.MsgId)).data, `message-${id}-audio.sil`);
                const options = {
                    attrNodeName: '$',
                    attributeNamePrefix: '',
            ...
            case PUPPET.types.Message.Video:
                ...
                return file_box_1.FileBox.fromStream((await this.wechat4u.getVideo(rawPayload.MsgId)).data, `message-${id}-video.mp4`);
            case PUPPET.types.Message.Attachment:
                if (rawPayload.AppMsgType === 6) {
                    ...
                    return file_box_1.FileBox.fromStream((await this.wechat4u.getDoc(rawPayload.FromUserName, rawPayload.MediaId, rawPayload.FileName)).data, rawPayload.FileName);
                }
                break;
            default:
                break;

      ...
      return Promise.resolve().then(function () {
        ...
        return _this4.request({
          ...
          if (error.response.status === 301) {
            ...
            var pm = data.match(/<ret>(.*)<\/ret>/);
            if (pm && pm[1] === '0') {
              _this4.PROP.skey = data.match(/<skey>(.*)<\/skey>/)[1];
              _this4.PROP.sid = data.match(/<wxsid>(.*)<\/wxsid>/)[1];
              _this4.PROP.uin = data.match(/<wxuin>(.*)<\/wxuin>/)[1];
              _this4.PROP.passTicket = data.match(/<pass_ticket>(.*)<\/pass_ticket>/)[1];
            }
            if (error.response.headers['set-cookie']) {
              error.response.headers['set-cookie'].forEach(function (item) {
                if (/webwx.*?data.*?ticket/i.test(item)) {
                  ...
                  _this4.PROP.passTicket = item.match(/=(.*?);/)[1];
                }
              });
            }
          } else {
